...
BigInt toUnit(String amount, {int decimals = 6}) {
  ...
  return BigInt.from((double.tryParse(amount) ?? 0) * BigInt.from(10).pow(exponent < 0 ? 0 : exponent).toDouble());
}

/// fromUnit takes a BigInt and converts it into a user readable amount
String fromUnit(BigInt amount, {int decimals = 6}) {
  final pow = decimals;
  return BigInt.from(amount / BigInt.from(10).pow(pow < 0 ? 0 : pow)).toString();
}

String fromDoubleUnit(String amount, {int decimals = 6}) {
  final exponent = decimals;
  return (double.parse(amount) / pow(10.0, exponent < 0 ? 0 : exponent)).toStringAsFixed(2);
}
...
EthPrivateKey? stringToPrivateKey(String privateKey) {
  try {
    final String formattedPrivateKey = privateKey.startsWith(hexPadding) ? privateKey : '$hexPadding$privateKey';
...
String generateSignature((String body, EthPrivateKey privateKey) args) {
  ...
  // sign the message
  final signature = args.$2.signPersonalMessageToUint8List(convertBytesToUint8List(hash));
...
EthereumAddress recoverAddressFromPersonalSignature(Uint8List originalData, Uint8List signature) {
