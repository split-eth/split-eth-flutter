...
class CardManagerContract {
  ...
  Future<void> init() async {
    final rawAbi = await rootBundle.loadString('packages/smartcontracts/contracts/external/CardFactory.abi.json');
  ...
  Uint8List withdrawCallData(Uint8List hash, String token, String to, BigInt amount) {
    final function = rcontract.function('withdraw');

    return function.encodeCall([hash, EthereumAddress.fromHex(token), EthereumAddress.fromHex(to), amount]);
  }
}
