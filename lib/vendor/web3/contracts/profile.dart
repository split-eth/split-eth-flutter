...
class ProfileContract {
  ...
  Future<void> init() async {
    final abi = await rootBundle.loadString('packages/smartcontracts/contracts/apps/Profile.abi.json');
  ...
  Future<String> getURLFromUsername(String username) async {
    return contract.getFromUsername(convertStringToUint8List(username, forcePadLength: 32));
  }
  ...
  Uint8List setCallData(String addr, String username, String url) {
    ...
    return function.encodeCall(
      [EthereumAddress.fromHex(addr), convertStringToUint8List(username, forcePadLength: 32), url],
    );
  }
